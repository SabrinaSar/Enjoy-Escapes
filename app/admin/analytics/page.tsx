...
export default async function ClickAnalytics({
  ...
  const [
    ...
    supabase
      .from("clicks_data")
      .select(
        `id, escape_id, created_at, source, referrer, user_agent, item_type`,
      )
  ...
  const totalEscapeClicks = Number(summary.total_escape_clicks || 0);
  const activeEscapesCount = Number(summary.active_escapes_count || 0);
  const clickCounts = summary.click_counts || [];
  const bannerClickCounts = summary.banner_click_counts || [];

  const totalEscapePages = Math.ceil(activeEscapesCount / pageSize);

  // Fetch escape details for items on current page + recent
  const escapeDetails: Record<number, EscapeDetails> = {};
  const currentEscapeIds = Array.from(
    new Set([
      ...(clickCounts
        ?.map((item: ClickCountData) => item.escape_id)
        .filter((id: number | undefined): id is number => id != null) || []),
      ...(recentClicks
        ?.filter(
          (click) =>
            !("item_type" in click) || (click as any).item_type === "escape",
        )
        .map((click) => click.escape_id) || []),
    ]),
  );

  if (currentEscapeIds.length > 0) {
    const { data: escapes } = await supabase
      .from("escapes_data")
      .select("id, title, price, link, type")
      .in("id", currentEscapeIds);
  ...
  const allBannerIds = Array.from(
    new Set([
      ...(bannerClickCounts
        ?.map((item: ClickCountData) => item.banner_id ?? item.escape_id)
        .filter((id: number | undefined): id is number => id != null) || []),
      ...(recentClicks
        ?.filter((click) => (click as any).item_type === "banner")
        .map((click) => click.escape_id) || []),
    ]),
  );
  ...
  const sortedEscapes = (clickCounts || []).map((item: ClickCountData) => {
    const id = item.escape_id || 0;
    return {
      escapeId: id,
  ...
  const sortedBanners = (bannerClickCounts || []).map(
    (item: ClickCountData) => {
      const id = item.banner_id ?? item.escape_id ?? 0;
      return {
        bannerId: id,
        count: item.click_count,
  ...
  const totalBannerClicks = sortedBanners.reduce(
    (sum: number, item: { count: number }) => sum + item.count,
    0,
  );

  const hasError = analyticsError || recentClicksError;
          ...
          <p>
            {
              ((analyticsError as any)?.message ||
                (recentClicksError as any)?.message) as string
            }
          </p>
          <div className="mt-4 p-3 bg-white/50 dark:bg-black/20 rounded border border-red-200 dark:border-red-800 text-sm">
            <h4 className="font-bold mb-1">Potential Fix:</h4>
            <p>
              If you see a "timeout" or "function does not exist" error, you
              need to apply the database optimization migration.
            </p>
            <p className="mt-1">
              Run the SQL script in <code>db/fix_analytics_timeout.sql</code>{" "}
    ...
    <div className="p-2 sm:p-4 space-y-6 sm:space-y-8">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-blue-800 dark:text-blue-200">
              Our analytics track <strong>every click</strong> that happens on
              your website. However, affiliate networks (like Awin) will show
              lower numbers because they filter clicks for:
            </p>
            <ul className="text-sm text-blue-800 dark:text-blue-200 list-disc ml-5 space-y-1">
              <li>
                <strong>Ad blockers:</strong> Users with ad blockers prevent
                affiliate tracking pixels from loading
              </li>
              <li>
                <strong>Privacy settings:</strong> Users who block cookies or
                have strict privacy settings
              </li>
              <li>
                <strong>Network issues:</strong> Clicks that don't successfully
                reach the affiliate network's servers
              </li>
              <li>
                <strong>Fraud prevention:</strong> Affiliate networks filter
                suspicious or duplicate clicks
              </li>
              <li>
                <strong>Session requirements:</strong> Some networks require
                JavaScript, specific browsers, or validated sessions
              </li>
              <li>
                <strong>Click deduplication:</strong> Multiple clicks from the
                same user may only count as one
              </li>
            </ul>
            <p className="text-sm text-blue-800 dark:text-blue-200 pt-2">
              <strong>This is normal!</strong> Expect affiliate networks to
              report 30-50% fewer clicks than your internal analytics. Your
              analytics provide the complete picture of user engagement, while
              affiliate networks show validated, commission-eligible clicks.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-card rounded-lg p-3 sm:p-4 shadow-sm border">
          ...
          <p className="text-xl sm:text-3xl font-bold mt-1 sm:mt-2">
            {activeEscapesCount + sortedBanners.length || 0}
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            {activeEscapesCount} escapes, {sortedBanners.length} banners
          </p>
        </div>
        ...
        <div className="bg-card rounded-lg p-3 sm:p-4 shadow-sm border">
          ...
          <p className="text-sm sm:text-base font-medium mt-1 sm:mt-2 truncate">
            {sortedEscapes.length > 0
              ? sortedEscapes[0].details.title
              : "No data"}
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            {sortedEscapes.length > 0
              ? `${sortedEscapes[0].count} clicks`
              : "Most clicked escape"}
          </p>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          ...
          <div className="text-sm text-muted-foreground">
            ...
            {totalBannerClicks} total clicks
          </div>
        </div>
        {sortedBanners.length > 0 ? (
            ...
            <div className="min-w-full px-2 sm:px-0">
              <div className="rounded-md border overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {sortedBanners.map((item: any, index: number) => (
                      <tr
                        key={item.bannerId}
                      ...
                      >
                        ...
                        <td className="py-2 sm:py-3 px-2 sm:px-4 font-bold text-primary">
                          {item.count}
                        </td>
                      </tr>
                    ))}
          ...
          <div className="py-8 text-center text-muted-foreground bg-muted/20 rounded-lg">
            ...
            <p className="text-xs mt-1">
              Banner clicks will appear here once users start interacting with
              promotional banners
            </p>
          </div>
        )}
      ...
      <div>
        ...
        {sortedEscapes.length > 0 ? (
            ...
            <div className="min-w-full px-2 sm:px-0">
              <div className="rounded-md border overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {sortedEscapes.map((item: any, index: number) => (
                      <tr
                        key={item.escapeId}
                        className="border-t hover:bg-muted/50"
                      >
                        ...
                        <td className="py-2 sm:py-3 px-2 sm:px-4">
                          {formatEscapeType(item.details.type)}
                        </td>
                        <td className="py-2 sm:py-3 px-2 sm:px-4">
          ...
          <div className="py-8 text-center text-muted-foreground bg-muted/20 rounded-lg">
            ...
            <p className="text-xs mt-1">
              Clicks will appear here once users start interacting with escapes
            </p>
          </div>
        )}
      ...
      <div>
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          ...
          <div className="text-sm text-muted-foreground">Last 10 clicks</div>
        </div>
        {recentClicks && recentClicks.length > 0 ? (
          <div className="overflow-x-auto -mx-2 sm:mx-0">
            <div className="min-w-full px-2 sm:px-0">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {recentClicks.map((click: ClickData) => {
                      ...
                      const itemDetails = isBanner
                        ? bannerDetails[click.escape_id]
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-2 sm:py-3 px-2 sm:px-4">
                            <span
                              className={`text-xs px-2 py-1 rounded ${isBanner ? "bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300" : "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300"}`}
                            >
                          ...
                          <td className="py-2 sm:py-3 px-2 sm:px-4">
                            <div className="flex flex-col">
                              <span className="font-medium">
                                {format(new Date(click.created_at), "MMM d")}
                              </span>
                              <span className="text-xs text-muted-foreground">
                                {format(new Date(click.created_at), "HH:mm")}
                              </span>
                            </div>
                          </td>
          ...
          <div className="py-8 text-center text-muted-foreground bg-muted/20 rounded-lg">
            ...
            <p className="text-xs mt-1">
              Recent clicks will appear here as users interact with escapes
            </p>
          </div>
        )}
      </div>
...
function processClicksByEscape(
  clicks: AllClicksData[],
