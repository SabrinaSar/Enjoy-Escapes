...
export default async function ClickAnalytics() {
  ...
  return (
    <div className="container mx-auto p-4 space-y-8">
      <h1 className="text-3xl font-bold">Escape Analytics</h1>
      
      {/* Overview Stats */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-card rounded-lg p-4 shadow-sm border">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-medium text-muted-foreground">Total Clicks</h3>
            <TrendingUp className="h-5 w-5 text-primary" />
          </div>
          <p className="text-3xl font-bold mt-2">{totalClicks || 0}</p>
        </div>
        
        <div className="bg-card rounded-lg p-4 shadow-sm border">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-medium text-muted-foreground">Top Escape</h3>
        ...
        <div className="bg-card rounded-lg p-4 shadow-sm border">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-medium text-muted-foreground">Recent Date</h3>
        ...
        <div className="bg-card rounded-lg p-4 shadow-sm border">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-medium text-muted-foreground">Tracked Escapes</h3>
            <ListFilter className="h-5 w-5 text-primary" />
          </div>
          <p className="text-3xl font-bold mt-2">{Object.keys(clicksByEscape).length || 0}</p>
        </div>
      </div>
      
      {/* Top Escapes Table */}
      <div>
        <h2 className="text-2xl font-bold mb-4">Top Escapes</h2>
        {sortedEscapes.length > 0 ? (
          <div className="rounded-md border overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-muted/50">
                <tr>
                  <th className="py-3 px-4 text-left font-medium">Escape</th>
              ...
              <tbody>
                {sortedEscapes.map((item) => (
                  <tr key={item.escapeId} className="border-t hover:bg-muted/50">
                    ...
                    <td className="py-3 px-4">{formatEscapeType(item.details.type)}</td>
                    <td className="py-3 px-4">£{item.details.price || 0}</td>
      ...
      {/* Recent Clicks Table */}
      <div>
        <h2 className="text-2xl font-bold mb-4">Recent Clicks</h2>
        {recentClicks && recentClicks.length > 0 ? (
          <div className="rounded-md border overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-muted/50">
                <tr>
                  ...
                  <th className="py-3 px-4 text-left font-medium">Referrer</th>
                </tr>
              </thead>
              <tbody>
                {recentClicks.map((click: ClickData) => (
                  <tr key={click.id} className="border-t hover:bg-muted/50">
                    ...
                    <td className="py-3 px-4">{format(new Date(click.created_at), 'PPp')}</td>
                    <td className="py-3 px-4 max-w-[200px] truncate">{click.source || '-'}</td>
                    <td className="py-3 px-4 max-w-[200px] truncate">{click.referrer || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
